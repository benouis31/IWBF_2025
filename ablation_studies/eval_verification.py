...
def test(model, dataloader,epoch,Output,is_graph=False):
    global best_test
    labels, distances = [], []
   ...
   # roc_auc = auc(fpr, tpr)
    ...
    if is_best and is_graph:
        plot_roc(fpr, tpr, figure_name=Output + '/Test_ROC-{}.png'.format(epoch))
        ...
        if args.evaluate is False:
            shutil.copyfile(Output + '/model_best.pth.tar', args.outdir + '/test_model_best.pth.tar')
...
if __name__ == '__main__':
    ...
    for SEED in SEEDS:
        ...
        model.eval()
        EER = test(model, test_dl, epoch,is_graph=True)
